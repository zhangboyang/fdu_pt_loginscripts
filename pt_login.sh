...
(wget -q -O - --load-cookies cookies.txt --save-cookies cookies.txt --keep-session-cookies 'https://pt.vm.fudan.edu.cn/index.php?action=login2' --post-data "user=$username&passwrd=&cookielength=1440&hash_passwrd=$hash_password" | grep "$username" | grep -q "action=profile") || \
(wget -q -O - --load-cookies cookies.txt --save-cookies cookies.txt --keep-session-cookies 'http://pt.vm.fudan.edu.cn/index.php?action=login2' --post-data "user=$username&passwrd=&cookielength=1440&hash_passwrd=$hash_password" | grep "$username" | grep -q "action=profile") || exit 1
